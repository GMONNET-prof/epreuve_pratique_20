...
#############################################################################
def calculer_empreinte(dico):
    total = 0
    for cle in EMISSIONS:
        for i in dico:
            if cle == i :
                total += dico[i] * EMISSIONS[cle]
    return total
...
#############################################################################
def classer_par_impact(dico):
    impact = {"fort":[],"moyen":[],"faible":[]}
    for cle in EMISSIONS:
        for i in dico:
            somme = 0
            if cle == i :
                somme += dico[i] * EMISSIONS[cle]
                if somme > 1000:
                    impact["fort"].append(i)
                elif somme < 200:
                    impact["faible"].append(i)
                else:
                    impact["moyen"].append(i)
    return impact
