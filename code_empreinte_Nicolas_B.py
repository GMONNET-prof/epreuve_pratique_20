...
def calculer_empreinte(utilisateur):
    ...
    for cle in utilisateur:
        assert(cle in EMISSIONS)==True,"erreure la cle n'est pas dans EMISSION"
        total+=EMISSIONS[cle]*utilisateur[cle]
...
def classer_par_impact(utilisateur):
    impacte={'fort':[],'moyen':[],'faible':[]}
    impacte_fort=0
    impacte_moy=0
    impacte_faible=0
    for cle in utilisateur:
        assert(cle in EMISSIONS)==True,"erreure la cle n'est pas dans EMISSION"
        emission=EMISSIONS[cle]*utilisateur[cle]
        if impacte_faible<emission and emission<200:
            impacte['faible']=[cle]
            impacte_faible=emission
        
        if impacte_moy<emission and 200<=emission <1000:
            impacte['moyen']=[cle]
            impacte_moy=emission
        
        if impacte_fort<emission and emission>=1000:
            impacte['fort']=[cle]
            impacte_fort=emission
