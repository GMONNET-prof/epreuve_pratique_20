...
def calculer_empreinte(utilisateur):
    total_emission = 0
    for cle, valeur in EMISSIONS.items():
        if cle in utilisateur:
            total_emission += valeur * utilisateur[cle]
    return total_emission
            
# print(calculer_empreinte(utilisateur5))
# print(calculer_empreinte(utilisateur1))
# print(len(utilisateur5))
#print(calculer_empreinte(utilisateur6))
#############################################################################
# Écrire le code de la fonction classer_par_impact de la question 2         #
#############################################################################
def classer_par_impact(utilisateur):
    total_emission = 0
    dico_impact = {'fort':[], 'moyen':[], 'faible':[]}
    for cle, valeur in EMISSIONS.items():
        if cle in utilisateur:
            total_emission = valeur * utilisateur[cle]
            if total_emission < 200:
                dico_impact['faible'].append(cle)
            elif total_emission >= 1000:
                dico_impact['fort'].append(cle)
            else:
                dico_impact['moyen'].append(cle)
    return dico_impact
...
#print(comparer_v2(utilisateur1, utilisateur2))
