...
def calculer_empreinte(utilisateur):
    ...
    # usage += utilisateur.get('emails_simples') * 4
    # usage += utilisateur.get('emails_pj') * 19
    # usage += utilisateur.get('streaming_sd') * 36
    # usage += utilisateur.get('streaming_hd') * 100
    # usage += utilisateur.get('recherches') * 7
    # usage += utilisateur.get('stockage_cloud') * 10
    
    for activite, co in EMISSIONS.items():
        usage += utilisateur.get(activite) * EMISSIONS.get(activite)
    

    return usage
print(calculer_empreinte(utilisateur1))
# assert(calculer_empreinte(utilisateur1)) == '7490 gCO2e'
#############################################################################
...
def classer_par_impact(utilisateur):
    emission_resultat = {
        ...
        'moyen': [] 
        'faible': []}
    for activite in utilisateur.keys:
        usage = 0
        if calculer_empreinte(activite) >= 1000:
            emission_resultat['fort'].append(activite)
        elif calculer_empreinte(activite) < 200:
            emission_resultat['faible'].append(activite)
        else:
            emission_resultat['moyen'].append(activite)
            
    return emission_resultat
